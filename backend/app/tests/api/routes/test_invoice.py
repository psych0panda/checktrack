...
from app.models import Invoice, Product, Payment, User
...
def test_print_invoice(client: TestClient, superuser_token_headers: dict[str, str], db: Session):
    # Создаем тестового пользователя с уникальным email
    user = User(id=uuid.uuid4(), email=random_email(), hashed_password=get_password_hash("password"))
    db.add(user)
    db.commit()
...
def test_read_invoices_with_filters(client: TestClient, superuser_token_headers: dict[str, str], db: Session):
    # Создаем тестового пользователя с уникальным email
    user = User(id=uuid.uuid4(), email=random_email(), hashed_password=get_password_hash("password"))
    db.add(user)
    db.commit()
